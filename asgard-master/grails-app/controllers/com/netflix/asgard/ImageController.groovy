...
class ImageController {
	...
	def show() {
		...
		if (!image) {
			...
			try {
				launchUsers = awsEc2Service.getImageLaunchers(userContext, image.imageId)
			}
			catch (AmazonServiceException ignored) {
				/* We may not own the image, so ignore failures here */
			}
	...
	def analyze() {
		...
		ImageControllerHelper imgHlpr = new ImageControllerHelper();
		def details = imgHlpr.getDetailMap(awsEc2Service, mergedInstanceGroupingService)
